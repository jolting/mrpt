...
#[no_mangle]
pub extern "C" fn mrpt_wrap_to_2pi(a: f64) -> f64 {
    crate::wrap2pi::wrap_to_2pi(a)
}

/// Wrap angle to (-π, π] range
#[no_mangle]
pub extern "C" fn mrpt_wrap_to_pi(a: f64) -> f64 {
    crate::wrap2pi::wrap_to_pi(a)
}

/// Compute shortest angular distance
#[no_mangle]
pub extern "C" fn mrpt_ang_distance(from: f64, to: f64) -> f64 {
    crate::wrap2pi::ang_distance(from, to)
}

/// Wrap angle to [0, 2π) range (f32)
#[no_mangle]
pub extern "C" fn mrpt_wrap_to_2pi_f32(a: f32) -> f32 {
    crate::wrap2pi::wrap_to_2pi_f32(a)
}

/// Wrap angle to (-π, π] range (f32)
#[no_mangle]
pub extern "C" fn mrpt_wrap_to_pi_f32(a: f32) -> f32 {
    crate::wrap2pi::wrap_to_pi_f32(a)
}

/// Compute shortest angular distance (f32)
#[no_mangle]
pub extern "C" fn mrpt_ang_distance_f32(from: f32, to: f32) -> f32 {
    crate::wrap2pi::ang_distance_f32(from, to)
}
...
/// and `buffer` points to valid memory of at least `buffer_len` bytes
#[no_mangle]
pub unsafe extern "C" fn mrpt_string_to_lower(
    input: *const c_char,
    buffer: *mut c_char,
    buffer_len: usize,
) -> c_int {
    ...
    let lower = crate::string_utils::to_lower(input_str);
    let lower_bytes = lower.as_bytes();

    if lower_bytes.len() >= buffer_len {
        return -1;
    }

    std::ptr::copy_nonoverlapping(lower_bytes.as_ptr(), buffer as *mut u8, lower_bytes.len());
    *buffer.add(lower_bytes.len()) = 0;

    lower_bytes.len() as c_int
}
...
/// and `buffer` points to valid memory of at least `buffer_len` bytes
#[no_mangle]
pub unsafe extern "C" fn mrpt_string_to_upper(
    input: *const c_char,
    buffer: *mut c_char,
    buffer_len: usize,
) -> c_int {
    ...
    let upper = crate::string_utils::to_upper(input_str);
    let upper_bytes = upper.as_bytes();

    if upper_bytes.len() >= buffer_len {
        return -1;
    }

    std::ptr::copy_nonoverlapping(upper_bytes.as_ptr(), buffer as *mut u8, upper_bytes.len());
    *buffer.add(upper_bytes.len()) = 0;

    upper_bytes.len() as c_int
}
