...
template <> const char * mrpt::utils::CSerializer<CEllipsoid>::getClassName() { return "CEllipsoid";}
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
template <> void CSerializer<CEllipsoid>::writeToStream(const CEllipsoid& o, mrpt::utils::CStream &out,int *version)
{
	if (version)
		*version = 1;
	else
