...
namespace mrpt
{
	...
	namespace utils
	{
		...
		class BASE_IMPEXP CSerializableCRTPVirtual : public BASE
		{
			...
			static const mrpt::utils::TRuntimeClassId runtimeClassId;// = {CSerializer<T>::getClassName(), &CSerializableCRTP<T>::CreateObject, &CSerializer<T>::getBaseClass };
			static const mrpt::utils::TRuntimeClassId *classinfo;// = {CSerializer<T>::getClassName(), &CSerializableCRTP<T>::CreateObject, &CSerializer<T>::getBaseClass };
