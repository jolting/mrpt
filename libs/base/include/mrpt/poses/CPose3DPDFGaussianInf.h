	...
	namespace poses
	{
		...
		class CPose3DQuatPDFGaussian;
			...
			/** Declares a class that represents a Probability Density function (PDF) of a 3D pose \f$ p(\mathbf{x}) = [x ~ y ~ z ~ yaw ~ pitch ~ roll]^t \f$ as a Gaussian described by its mean and its inverse covariance matrix.
	 		 ...
	 		 */
		class BASE_IMPEXP CPose3DPDFGaussianInf : public mrpt::utils::CSerializableCRTP<CPose3DPDFGaussianInf, CPose3DPDF>
		{
		  ...
		  */
		   ...
		   * \sa getMean  */
			...
			virtual void getInformationMatrix(mrpt::math::CMatrixDouble66 &inf) const override { inf=cov_inv; }
