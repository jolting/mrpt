...
extern "C" {
int mrpt_exception_line_msg(
    const char* msg, const char* filename, uint32_t line, 
    const char* function_name, char* out_buffer, size_t buffer_len);
}
    ...
    const std::string_view& msg, const char* filename, unsigned int line, const char* function_name)
{
  ...
  std::string buffer;
  buffer.resize(4096); // Allocate reasonable buffer
  
  int result = mrpt_exception_line_msg(
  ...
  // Fallback to C++ implementation if Rust fails
  std::string s;
  s += filename;
  s += ":";
