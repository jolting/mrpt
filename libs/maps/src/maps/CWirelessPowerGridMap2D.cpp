...
template <> const char * mrpt::utils::CSerializer<CWirelessPowerGridMap2D>::getClassName() { return "CWirelessPowerGridMap2D";}
...
CWirelessPowerGridMap2D::CWirelessPowerGridMap2D(
	...
	double y_max,
	double resolution ) :
		mrpt::utils::CSerializableCRTP<CWirelessPowerGridMap2D, CRandomFieldGridMap2D>(mapType, x_min,x_max,y_min,y_max,resolution ),
		insertionOptions()
{
...
/*---------------------------------------------------------------
 ...
 ---------------------------------------------------------------*/
template <> void CSerializer<CWirelessPowerGridMap2D>::writeToStream(const CWirelessPowerGridMap2D &o, mrpt::utils::CStream &out, int *version)
{
	if (version)
		*version = 5;
	else
	...
	case 5:
		{
...
#if MRPT_IS_BIG_ENDIAN
				for (uint32_t i=0;i<n;i++)
					in >> o.m_map[i].kf_mean >> o.m_map[i].dm_mean >> o.m_map[i].dmv_var_mean;
#else
			...
			if (version>=1)
			{
				uint8_t	i;
				in  >> i;
				o.m_mapType = CWirelessPowerGridMap2D::TMapRepresentation(i);
