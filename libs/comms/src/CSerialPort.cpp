...
void CSerialPort::open()
{
  ...
  if (INVALID_HANDLE_VALUE == (hCOM = CreateFileA(
    ...
    int baudRate, int parity, int bits, int nStopBits, bool enableFlowControl)
{
  ...
  // Apply:
  if (!SetCommState(hCOM, &dcb_conf)) THROW_EXCEPTION("Error changing COM state");
  ...
  if (special_rate)
  {
    ...
    if (ioctl(hCOM, TIOCGSERIAL, &serial) < 0) THROW_EXCEPTION("error on TIOCGSERIAL ioctl");
    ...
    int WriteTotalTimeoutConstant)
{
  ...
  if (!SetCommTimeouts(hCOM, &timeouts)) THROW_EXCEPTION("Error changing COM port timeout config");
