...
template <> const char * mrpt::utils::CSerializer<CMultiObjectiveMotionOptimizerBase>::getClassName() { return "CMultiObjectiveMotionOptimizerBase";}
	...
	for (unsigned int mov_idx = 0; mov_idx < movs.size(); ++mov_idx)
	{
		...
		bool assert_failed = false;
		{
			for (auto &ma : m_movement_assert_exprs)
			{
				...
				if (val == 0) {
					assert_failed = true;
					extra_info.log_entries.emplace_back(mrpt::format("[CMultiObjectiveMotionOptimizerBase] mov_idx=%u ASSERT failed: `%s`", mov_idx, ma.get_original_expression().c_str()));
					break;
				}
			}
		}
