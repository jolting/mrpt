	...
	if( m_showPreview )
	{
		...
		if( m_win && m_win->isOpen() )
		{
			COpenGLScene::Ptr & scene = m_win->get3DSceneAndLock();
			{
				...
				{
					CSetOfObjects::Ptr dummy = static_cast<CSetOfObjects::Ptr>( scene->getByName("dummy") );
					dummy->setVisibility(true);
				}

				// update joints positions
				CSetOfObjects::Ptr body = static_cast<CSetOfObjects::Ptr>( scene->getByName("body") );
				ASSERT_( body )
							
				for(int i = 0; i < NUM_JOINTS; ++i)
				{
					CSphere::Ptr s = static_cast<CSphere::Ptr>( body->getByName( jointNames[i] ) );
					CPoint3D sphPos;
					if( i == 0 )
	...
	if( m_showPreview )
	{
		...
		if( m_win && m_win->isOpen() )
		{
...

			{
				...
				CSetOfLines::Ptr lines = static_cast<CSetOfLines::Ptr>(body->getByName("lines")  );
				ASSERT_(lines)
				...
				for(int i = 0; i < NUM_LINES; ++i)
				{
					...
					CSphere::Ptr s0 = static_cast<CSphere::Ptr>( body->getByName( jointNames[pair.first] ) );
					CSphere::Ptr s1 = static_cast<CSphere::Ptr>( body->getByName( jointNames[pair.second] ) );
					ASSERT_(s0 && s1)
