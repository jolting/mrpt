...
CCameraSensor::CCameraSensor() :
	...
	m_cap_cv             (nullptr),
	m_cap_dc1394         (nullptr),
	m_cap_flycap         (nullptr),
	m_cap_flycap_stereo_l(nullptr),
	m_cap_flycap_stereo_r(nullptr),
	m_cap_bumblebee_dc1394(nullptr),
	m_cap_svs            (nullptr),
	m_cap_ffmpeg         (nullptr),
	m_cap_rawlog         (nullptr),
	m_cap_swissranger    (nullptr),
	m_cap_kinect         (nullptr),
	m_cap_openni2        (nullptr),
	m_cap_image_dir		(nullptr),
	m_cap_duo3d			(nullptr),
...
CObservation::Ptr CCameraSensor::getNextFrame( )
{
	...
	return static_cast<CObservation::Ptr>(out_obs[0]);
}
	...
	while (!m_threadImagesSaverShouldEnd)
	{
		...
		for (TListObservations::const_iterator i=newObs.begin();i!=newObs.end();++i)
		{
			...
			if (m_hook_pre_save)
			{
				...
				{
					mrpt::obs::CObservation::Ptr obs = mrpt::obs::CObservation::Ptr(i->second);
			...
			{
				CObservationImage::Ptr obs = CObservationImage::Ptr(i->second);
			...
			{
				CObservationStereoImages::Ptr stObs = CObservationStereoImages::Ptr(i->second);
