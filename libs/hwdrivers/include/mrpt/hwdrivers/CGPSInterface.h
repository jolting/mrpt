...
namespace mrpt
{
	namespace hwdrivers
	{
		...
		public:
			...
			//void setExternCOM( CSerialPort *outPort, mrpt::synch::std::mutex *csOutPort ); // Replaced by bindStream() in MRPT 1.4.0
			...
			/** This enforces the use of a given user stream, instead of trying to open the serial port set in this class parameters.
			  ...
			  */
			void bindStream(mrpt::utils::CStream * external_stream, mrpt::synch::std::mutex *csOptionalExternalStream = nullptr );
		...
		protected:
			...
			mrpt::synch::std::mutex  *m_data_stream_cs;
