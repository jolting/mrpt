...
tuple CICP_AlignPDF1(CICP &self, COccupancyGridMap2D &m1, CSimplePointsMap &m2, CPosePDFGaussian &initialEstimationPDF)
{
    ...
    CPosePDF::Ptr posePDF::Ptr = self.AlignPDF(&m1, &m2, initialEstimationPDF, &runningTime, &info);
    posePDF.copyFrom(*posePDF::Ptr);
...
{
    return (CPose3DPDFParticles::Ptr) self.getCurrentPoseEstimation();
}
