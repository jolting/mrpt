	...
	if (obj->GetRuntimeClass()==CLASS_ID(CHMHMapNode))
	{
		...
		for (int nRound=0;nRound<2;nRound++)
		{
			...
			for (size_t  nSelItem = 0; nSelItem<nSel;nSelItem++)
			{
				...
				CHMHMapNode::Ptr area= CHMHMapNode::Ptr(data1->m_ptr);
				if (!area) continue;
...
void hmtMapViewerFrame::OnMenuOverlapBtw2(wxCommandEvent& event)
{
	...
	CHMHMapNode::Ptr area1=CHMHMapNode::Ptr(data1->m_ptr);
	CHMHMapNode::Ptr area2=CHMHMapNode::Ptr(data2->m_ptr);
...
void hmtMapViewerFrame::OnTopologicalModel_Gridmap(wxCommandEvent& event)
{
	...
	const CHMHMapNode::Ptr area1=CHMHMapNode::Ptr(data1->m_ptr);
	const CHMHMapNode::Ptr area2=CHMHMapNode::Ptr (data2->m_ptr);

	CTopLCDetectorBase::Ptr TLCD = CTopLCDetectorBase::Ptr( hmt_map->loopClosureDetector_factory("gridmaps") );
	...
	CPose3DPDFSOG::Ptr pdfSOG = CPose3DPDFSOG::Ptr(pdf);
