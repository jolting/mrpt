	...
	for (progress_i=0 ;it!=rawlog.end();progress_i--)
	{
		...
		if (it.getType()==CRawlog::etActionCollection)
		{
			...
			if (lastAct)
			{
				...
				CActionRobotMovement2D::Ptr act = CActionCollection::Ptr(*it)->getMovementEstimationByType( CActionRobotMovement2D::emOdometry );
				ASSERT_(act);
			...
			else
			{
				...
				lastAct = CActionCollection::Ptr(*it)->getMovementEstimationByType( CActionRobotMovement2D::emOdometry );
				ASSERT_(lastAct);
		...
		else if (it.getType()==CRawlog::etSensoryFrame)
		{
			...
			if (lastSF)
			{
				...
				lastSF->moveFrom( * CSensoryFrame::Ptr(*it) );
...
void xRawLogViewerFrame::OnMenuLossLessDecimate(wxCommandEvent& event)
{
	...
	for (i=0;i<N;i++)
	{
		...
		else if (rawlog.getType(i)==CRawlog::etSensoryFrame)
		{
			...
			accum_sf->moveFrom( *CSensoryFrame::Ptr(obj) );
	...
	while (keepLoading)
	{
		...
		try
		{
			...
			if ( newObj->GetRuntimeClass() == CLASS_ID(CSensoryFrame) )
			{
				...
				// Copy pointers to observations only (fast):
				accum_sf->moveFrom( *CSensoryFrame::Ptr(newObj) );
			...
			else if ( newObj->GetRuntimeClass() == CLASS_ID(CActionCollection) )
			{
				// Accumulate Actions
				// ----------------------
				CActionCollection::Ptr curActs = CActionCollection::Ptr( newObj );
				CActionRobotMovement2D::Ptr mov = curActs->getBestMovementEstimation();
				if (mov)
				{
		...
		try
		{
			...
			if ( newObj->GetRuntimeClass() == CLASS_ID(CSensoryFrame) )
			{
				...
				if ( newObj->GetRuntimeClass() == CLASS_ID(CActionCollection) )
				{
					...
					CActionCollection::Ptr	acts = CActionCollection::Ptr( newObj );
					// Get odometry:
				...
				{
					CObservation::Ptr o = CObservation::Ptr( newObj );
	...
	for (unsigned int countLoop=0;countLoop<nEntries;countLoop++)
	{
		...
		switch (rawlog.getType(countLoop))
		{
			...
			case CRawlog::etObservation:
			{
				...
				{
					cur_sf_odo = CObservationOdometry::Ptr(o);
				}
