			...
			case CRawlog::etObservation:
				{
					...
					{
						obs = CObservationGasSensors::Ptr(o);	//Get the GAS observation
					}
				}
				break;
			...
			case CRawlog::etObservation:
				{
					...
					{
						obs = CObservationWirelessPower::Ptr(o);	//Get the GAS observation
					}
				}
				break;
			...
			case CRawlog::etObservation:
				{
					...
					{
						obs = CObservationRFID::Ptr(o);	//Get the GAS observation
					}
				}
				break;
	...
	while (fileParser.getNextLine(line))
	{
		...
		if (words[0]=="STEP")
		{
			...
			else
			{
				...
				newRawlog.addObservationMemoryReference( CObservationOdometry::Ptr( new CObservationOdometry( obs_odo )));
			}
		}
		else
		...
		for (i=0;i<n;i++)
		{
			switch (rawlog.getType(i))
			{
				...
				case CRawlog::etObservation:
				{
					...
					{
						CObservationIMU::Ptr obs = CObservationIMU::Ptr(oo);
						// For each entry in this sequence:
						if (obs->rawMeasurements.size()>0)
						{
							ASSERT_( obs->dataIsPresent.size()==obs->rawMeasurements.size() );
	...
	if ( dialog.ShowModal() == wxID_OK )
	{
		...
		for (i=0;i<n;i++)
		{
			...
			else if ( rawlog.getType(i)==CRawlog::etObservation )
			{
				...
				{
					CObservationBearingRange::Ptr obs = CObservationBearingRange::Ptr(o);
					// For each entry in this sequence:
					for (size_t q=0;q<obs->sensedData.size();q++)
					{
						M++;
