		...
		try
		{
			...
			if ( newObj->GetRuntimeClass() == CLASS_ID(CSensoryFrame))
			{
				if ( entryIndex>=first && (last==-1 || entryIndex<=last)  )
					rawlog.addObservationsMemoryReference( CSensoryFrame::Ptr(newObj) );
				entryIndex++;
			}
			else if ( newObj->GetRuntimeClass() == CLASS_ID(CActionCollection))
			{
				if ( entryIndex>=first && (last==-1 || entryIndex<=last)  )
					rawlog.addActionsMemoryReference( CActionCollection::Ptr(newObj) );
				entryIndex++;
			}
			...
			{
				if ( entryIndex>=first && (last==-1 || entryIndex<=last)  )
					rawlog.addObservationMemoryReference( CObservation::Ptr(newObj) );
				entryIndex++;
			}
			...
			else if ( newObj->GetRuntimeClass() == CLASS_ID(CPose2D))
			{
				if ( entryIndex>=first && (last==-1 || entryIndex<=last)  )
				{
					CPose2D::Ptr poseChange = CPose2D::Ptr(newObj);
			...
			{
				CRawlog::Ptr rw = CRawlog::Ptr(newObj);
				rawlog.moveFrom(*rw);
			}
			else
			{
	...
	for (unsigned int i=0;i<rawlog.size();i++)
	{
		...
		switch ( rawlog.getType(i) )
		{
			...
			case CRawlog::etSensoryFrame:
			{
				...
				CSensoryFrame::Ptr sf = CSensoryFrame::Ptr( obj );
			...
			case CRawlog::etObservation:
			{
				...
				{
					CObservationOdometry::Ptr odoObs = CObservationOdometry::Ptr(obs);
		...
		for (i=0;i<n;i++)
		{
			switch( rawlog.getType(i) )
			{
				...
				case CRawlog::etObservation:
				{
					...
					CObservationBeaconRanges::Ptr obs = CObservationBeaconRanges::Ptr( o );
					::fprintf(f,"%u ", i);
		...
		case CRawlog::etSensoryFrame:
			{
				...
				for (unsigned int k=0;k<sf->size();k++)
				{
					...
					{
						CObservation2DRangeScan::Ptr obs = sf->getObservationByIndexAs<CObservation2DRangeScan::Ptr>(k);
		...
		case CRawlog::etObservation:
			{
				...
				{
					CObservation2DRangeScan::Ptr obs = CObservation2DRangeScan::Ptr( o );
				...
				{
					CObservationOdometry::Ptr odo = CObservationOdometry::Ptr(o);
...
void xRawLogViewerFrame::OnFileSaveImages(wxCommandEvent& event)
{
	...
	while (keepLoading)
	{
		...
		try
		{
			...
			if ( newObj->GetRuntimeClass() == CLASS_ID(CSensoryFrame) )
			{
				CSensoryFrame::Ptr SF = CSensoryFrame::Ptr( CSerializable::Ptr(newObj) );
...
void wxStaticBitmapPopup::OnPopupSaveImage(wxCommandEvent& event)
{
	try
	{
		...
		CObservation::Ptr curSelectedObservation = theMainWindow->curSelectedObservation;
...
void xRawLogViewerFrame::OnDecimateRecords(wxCommandEvent& event)
{
	...
	for (i=0;i<N;i++)
	{
		...
		if (rawlog.getType(i)==CRawlog::etActionCollection)
		{
			// Accumulate Actions
			// ----------------------
			CActionCollection::Ptr curActs = CActionCollection::Ptr( obj);
		...
		try
		{
			if ( rawlog.getType(countLoop) == CRawlog::etSensoryFrame )
			{
				...
				for (size_t k=0;k<SF->size();k++)
				{
					if (SF->getObservationByIndex(k)->GetRuntimeClass()==CLASS_ID(CObservation2DRangeScan) )
					{
						CObservation2DRangeScan::Ptr obsScan = SF->getObservationByIndexAs<CObservation2DRangeScan::Ptr>(k);
			...
			else if ( rawlog.getType(countLoop) == CRawlog::etObservation )
			{
				...
				{
					CObservation2DRangeScan::Ptr obsScan = CObservation2DRangeScan::Ptr(o);
					bool thisValid=false;
		...
		try
		{
            ...
            case CRawlog::etSensoryFrame:
				...
				for (size_t k=0;k<SF->size();k++)
				{
					...
					{
						CObservationGasSensors::Ptr obs = SF->getObservationByIndexAs<CObservationGasSensors::Ptr>(k);
		...
		try
		{
			if (rawlog.getType(countLoop) == CRawlog::etActionCollection )
			{
				...
				for (size_t k=0;k<acts->size();k++)
				{
					...
					{
						CActionRobotMovement2D::Ptr a = CActionRobotMovement2D::Ptr( act );
...
void doFilterErrScans(CObservation::Ptr &obs, size_t &invalidSegments, size_t &invalidRanges, bool &touched)
{
	if (obs->GetRuntimeClass()->derivedFrom( CLASS_ID( CObservation2DRangeScan ) ))
	{
		CObservation2DRangeScan::Ptr obsScan = CObservation2DRangeScan::Ptr(obs);
			...
			else if (rawlog.getType(countLoop)== CRawlog::etSensoryFrame )
			{
				...
				if (SF_new && SF_ref && act_between)
				{
					...
					if (icpInfo.goodness>minICPGoodness)
					{
						...
						CActionRobotMovement2D::Ptr act=act_between->getMovementEstimationByType(CActionRobotMovement2D::emScan2DMatching);
						if (!act)
						{
							// Create:
					...
					else
					{
						...
						for (int k=((int)act_between->size())-1;k>=0;k--)
						{
							...
							{
								CActionRobotMovement2D::Ptr act= CActionRobotMovement2D::Ptr( act_between->get(k) );
...
void xRawLogViewerFrame::OnMenuItem47Selected(wxCommandEvent& event)
{
	...
	if (curSelectedObject)
	{
		...
		{
			CActionCollection::Ptr acts = CActionCollection::Ptr( curSelectedObject );
	...
	if ( dialog.ShowModal() )
	{
...

		{
			...
			for (size_t i=0;i<rawlog.size();i++)
			{
				...
				{
					CObservationOdometry::Ptr obs = CObservationOdometry::Ptr(rawlog.getAsGeneric(i));
		...
		for (i=0;i<n;i++)
		{
			if ( rawlog.getType(i) == CRawlog::etSensoryFrame )
			{
				...
				for (size_t k=0;k<sf->size();k++)
				{
					...
					{
						CObservationRange::Ptr obs = sf->getObservationByIndexAs<CObservationRange::Ptr>(k);
			...
			else if ( rawlog.getType(i) == CRawlog::etObservation )
			{
				...
				{
					CObservationRange::Ptr obs = CObservationRange::Ptr(o);
		...
		for (size_t i=0;i<rawlog.size();i++)
		{
			if (rawlog.getType(i)==CRawlog::etActionCollection)
			{
				...
				for (unsigned int j=0;j<acts->size();j++)
				{
					...
					{
						CActionRobotMovement2D::Ptr actMov = CActionRobotMovement2D::Ptr( act );
						...
						if (actMov->estimationMethod == CActionRobotMovement2D::emScan2DMatching )
						{
							...
							CPosePDFGaussian::Ptr aux = CPosePDFGaussian::Ptr( actMov->poseChange.get_ptr() );
							aux->cov.zeros();
							aux->cov(0,0) =
								aux->cov(1,1) = std_xy2;
	...
	if (dialog.ShowModal())
	{
		...
		for (CRawlog::iterator it = rawlog.begin(); it!=rawlog.end() && keepDoing; it++,countLoop++)
		{
			...
			while (true)
			{
				...
				{
					obs = CObservation::Ptr(*it);
					isSingleObs = true;
				}
				else if (it.getType()==CRawlog::etSensoryFrame)
				{
					CSensoryFrame::Ptr sf = CSensoryFrame::Ptr(*it);
				...
				if (obs)
				{
					...
					if (c!=desiredCamParams.end())
					{
						...
						CObservationImage::Ptr img = CObservationImage::Ptr(obs);
		...
		for (CRawlog::iterator it = rawlog.begin(); it!=rawlog.end() && keepDoing; it++,countLoop++)
		{
			...
			while (true)
			{
				...
				if (it.getType()==CRawlog::etObservation)
				{
					...
					{
						obs = CObservation2DRangeScan::Ptr(*it);
						isSingleObs = true;
					}
		...
		case CRawlog::etSensoryFrame:
			{
				...
				for (size_t j=0;j<SF->size();j++)
				{
					...
					{
						CObservationBearingRange::Ptr o = CObservationBearingRange::Ptr(obs);
	...
	for (int countLoop=0;countLoop<nEntries;countLoop++)
	{
		...
		try
		{
			...
			if ( rawlog.getType(countLoop) == CRawlog::etSensoryFrame )
			{
				...
				{
					CObservationStereoImages::Ptr obsStereoImg = SF->getObservationByClass<CObservationStereoImages>();
					if (obsStereoImg)
					{
			...
			else if ( rawlog.getType(countLoop) == CRawlog::etObservation )
			{
				...
				{
					CObservationImage::Ptr obsImg = CObservationImage::Ptr(o);
					imgsForVideo.insert( TImageToSaveData(&obsImg->image, "IMAGE") );
				}
				else if (IS_CLASS(o,CObservationStereoImages))
				{
					CObservationStereoImages::Ptr obsStereoImg = CObservationStereoImages::Ptr(o);
					imgsForVideo.insert( TImageToSaveData(&obsStereoImg->imageLeft, "LEFT") );
					if (obsStereoImg->hasImageRight) imgsForVideo.insert( TImageToSaveData(&obsStereoImg->imageRight, "RIGHT") );
					if (obsStereoImg->hasImageDisparity) imgsForVideo.insert( TImageToSaveData(&obsStereoImg->imageDisparity, "DISP") );
				}
				...
				{
					CObservation3DRangeScan::Ptr obs3D = CObservation3DRangeScan::Ptr(o);
					imgsForVideo.insert( TImageToSaveData(&obs3D->intensityImage, "INTENSITY") );
				}
			} // end for each entry
		...
		try
		{
			if (rawlog.getType(countLoop) == CRawlog::etActionCollection )
			{
				...
				for (size_t k=0;k<acts->size();k++)
				{
					CAction::Ptr act = acts->get(k);
					if (act->GetRuntimeClass() == CLASS_ID( CActionRobotMovement2D ) )
						lastOdo = CActionRobotMovement2D::Ptr( act );
				}
			}
			else if (rawlog.getType(countLoop) == CRawlog::etSensoryFrame)
			{
...
void xRawLogViewerFrame::OnMenuRenameSingleObs(wxCommandEvent& event)
{
	...
	CObservation::Ptr obj = CObservation::Ptr(curSelectedObject);
