...
void xRawLogViewerFrame::SelectObjectInTreeView( const CSerializable::Ptr & sel_obj )
{
	...
	if (sel_obj)
	{
		...
		if ( classID->derivedFrom( CLASS_ID( CObservation ) ) )
		{
			...
			{
				CObservation2DRangeScan::Ptr obs_scan2d = CObservation2DRangeScan::Ptr(sel_obj);
			...
			curSelectedObservation = CObservation::Ptr( sel_obj );
		}
		if ( classID->derivedFrom( CLASS_ID( CAction ) ) )
		{
			CAction::Ptr obs( sel_obj );
		...
		if ( classID  == CLASS_ID(CObservation2DRangeScan) )
		{
			...
			CObservation2DRangeScan::Ptr obs = CObservation2DRangeScan::Ptr( sel_obj );
		...
		if ( classID  == CLASS_ID(CObservationImage) )
		{
			...
			CObservationImage::Ptr obs = CObservationImage::Ptr( sel_obj );
		...
		if ( classID  == CLASS_ID(CObservationStereoImages) )
		{
			...
			CObservationStereoImages::Ptr obs = CObservationStereoImages::Ptr(sel_obj);
		...
		if ( classID == CLASS_ID(CActionRobotMovement2D) )
		{
			...
			CActionRobotMovement2D::Ptr act = CActionRobotMovement2D::Ptr( sel_obj );
			...
			{
				CPosePDFParticles::Ptr aux = CPosePDFParticles::Ptr( act->poseChange.get_ptr() );
		...
		if ( classID  == CLASS_ID(CObservationBearingRange) )
		{
			...
			CObservationBearingRange::Ptr obs = CObservationBearingRange::Ptr( sel_obj );
		...
		if ( classID  == CLASS_ID(CActionRobotMovement3D) )
		{
			...
			// ----------------------------------------------------------------------
			//Notebook1->ChangeSelection( 1 );
			CActionRobotMovement3D::Ptr act = CActionRobotMovement3D::Ptr( sel_obj );
			cout << "Robot Movement (as a gaussian pose change):\n";
		...
		if ( classID  == CLASS_ID(CObservation3DRangeScan) )
		{
			...
			CObservation3DRangeScan::Ptr obs = CObservation3DRangeScan::Ptr( sel_obj );
		...
		if ( classID  == CLASS_ID(CObservationVelodyneScan) )
		{
			...
			// ----------------------------------------------------------------------
			Notebook1->ChangeSelection( 9 );
			CObservationVelodyneScan::Ptr obs = CObservationVelodyneScan::Ptr( sel_obj );
