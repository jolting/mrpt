...
void CScanMatching::OnbtnICPClick(wxCommandEvent&)
{
	...
	{
		CSensoryFrame::Ptr SF_ref = CSensoryFrame::Ptr(obj_ref);
		SF_ref->insertObservationsInto(refMap);
	}
	else if (IS_DERIVED(obj_ref,CObservation))
	{
		CObservation::Ptr	obs_ref = CObservation::Ptr(obj_ref);
...

	{
		...
		while (curStep <= maxSteps)
		{
			...
			{
				CPosePDFSOG::Ptr SOG = CPosePDFSOG::Ptr(poseEst);
				size_t  i,n=SOG->size();
