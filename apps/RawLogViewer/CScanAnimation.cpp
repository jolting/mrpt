...
void CScanAnimation::BuildMapAndRefresh(CSensoryFrame *sf)
{
	...
	for (CSensoryFrame::iterator it=sf->begin();it!=sf->end();++it)
	{
		...
		{
			CObservation3DRangeScan::Ptr o= CObservation3DRangeScan::Ptr(*it);
	...
	for (CSensoryFrame::iterator it=sf->begin();it!=sf->end();++it)
	{
		...
		{
			CObservation2DRangeScan::Ptr obs = CObservation2DRangeScan::Ptr(*it);
			wereScans = true;
			...
			if (it_gl!=m_gl_objects.end())
			{
				...
				CPlanarLaserScan::Ptr(ro.obj)->setScan(*obs);
		...
		if (IS_CLASS(*it,CObservation3DRangeScan))
		{
			CObservation3DRangeScan::Ptr obs = CObservation3DRangeScan::Ptr(*it);
			wereScans = true;
			if (tim_last==INVALID_TIMESTAMP || tim_last<obs->timestamp)
				tim_last = obs->timestamp;
			...
			if (it_gl!=m_gl_objects.end())
			{
				// Update existing object:
				TRenderObject &ro = it_gl->second;
				CPointCloudColoured::Ptr gl_obj = CPointCloudColoured::Ptr(ro.obj);
...
//				m_lstScans[obs->sensorLabel] = obs;
		...
		{
			CObservationVelodyneScan::Ptr obs = CObservationVelodyneScan::Ptr(*it);
			wereScans = true;
			if (tim_last==INVALID_TIMESTAMP || tim_last<obs->timestamp)
				tim_last = obs->timestamp;
