...
void navlog_viewer_GUI_designDialog::loadLogfile(const std::string &filName)
{
	...
	for (;;)
	{
		try
		{
			...
			{
				const CLogFileRecord::Ptr logptr = CLogFileRecord::Ptr(obj);
...
void navlog_viewer_GUI_designDialog::OnslidLogCmdScroll(wxScrollEvent& event)
{
	...
	CLogFileRecord::Ptr logptr = CLogFileRecord::Ptr(m_logdata[i]);
		...
		// Update 3D view:
		{
			...
			mrpt::opengl::CSetOfObjects::Ptr gl_robot_frame;
			{
				...
				if (!gl_rbframe_r) {
					...
					gl_robot_frame = mrpt::opengl::CSetOfObjects::Ptr(gl_rbframe_r);
				}
			...
			// Extrapolated poses from delay models:
			{
				...
				if (!gl_relposes_r) {
					...
					gl_relposes = mrpt::opengl::CSetOfObjects::Ptr(gl_relposes_r);
				}
...

			{
				...
				if (!gl_obs_r) {
					...
					gl_obs = mrpt::opengl::CPointCloud::Ptr(gl_obs_r);
				}
...

			{
				...
				if (!gl_path_r) {
					...
					gl_path = mrpt::opengl::CSetOfLines::Ptr(gl_path_r);
				}
					...
					if (ptg)
					{
						...
						}
						{
							...
							if (!gl_shape_r) {
								...
								gl_shape = mrpt::opengl::CSetOfLines::Ptr(gl_shape_r);
							}
							gl_shape->clear();
			...
			}
			{
				...
				if (!gl_trg_r) {
					...
					gl_trg = mrpt::opengl::CPointCloud::Ptr(gl_trg_r);
				}
	...
	for (unsigned int nPTG=0;nPTG<log.infoPerPTG.size();nPTG++)  // log.infoPerPTG.size() may be != nPTGs in the last entry is used for "NOP cmdvel"
	{
...

		{
			...
			// Chosen direction:
			{
				...
				auto gl_obj = mrpt::opengl::CSetOfLines::Ptr(scene->getByName("tp_selected_dir"));
				gl_obj->clear();
				gl_obj->appendLine(
					0, 0, 0,
			...
			{
				auto gl_obj = mrpt::opengl::CSetOfLines::Ptr(scene->getByName("tp_obstacles"));
				gl_obj->clear();
				if (nAlphas>2)
				{
			...
			// Target:
			{
				auto gl_obj = mrpt::opengl::CPointCloud::Ptr(scene->getByName("tp_target"));
				gl_obj->clear();
			...
			{
				auto gl_obj = mrpt::opengl::CPointCloud::Ptr(scene->getByName("tp_robot"));
				gl_obj->clear();
			...
			// Clearance-diagram:
			{
				auto gl_obj = mrpt::opengl::CMesh::Ptr(scene->getByName("tp_clearance"));
			...
			// Clearance-diagram:
			{
				auto gl_obj1 = mrpt::opengl::CSetOfLines::Ptr(scene->getByName("score_phase1"));
				auto gl_obj2 = mrpt::opengl::CSetOfLines::Ptr(scene->getByName("score_phase2"));
...
void navlog_viewer_GUI_designDialog::OnmnuMatlabPlotsSelected(wxCommandEvent& event)
{
	...
	{
        const CLogFileRecord::Ptr logsptr = CLogFileRecord::Ptr( m_logdata[i] );
