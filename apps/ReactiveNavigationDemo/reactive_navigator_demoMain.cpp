...
void reactive_navigator_demoframe::simulateOneStep(double time_step)
{
	...
	if (sel_PTG>=0 && sel_PTG<(int)lfr.infoPerPTG.size())
	{
		...
		{
			CLogFileRecord_ND::Ptr log = CLogFileRecord_ND::Ptr(lfr.infoPerPTG[sel_PTG].HLFR);
	...
	case 0:
		{
			...
			// Opengl viz:
			create_viz_robot_diff( *mrpt::opengl::CSetOfObjects::Ptr( gl_robot->getByName("robot_render") ) );
	...
	case 1:
		{
			...
			// Opengl viz:
			create_viz_robot_holo(*mrpt::opengl::CSetOfObjects::Ptr( gl_robot->getByName("robot_render") ));
