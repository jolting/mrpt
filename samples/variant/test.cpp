...
void thread_reader(CPipeReadEndPoint &read_pipe)
{
	try
	{
		...
		auto var = read_pipe.ReadVariant<mrpt::poses::CPose2D, mrpt::poses::CPose3D>();
		var.match([](auto &pose) { cout << "RX pose: " << pose << endl;});
		var = read_pipe.ReadVariant<mrpt::poses::CPose2D, mrpt::poses::CPose3D>();
		var.match([](auto &pose) { cout << "RX pose: " << pose << endl;});
...
void thread_writer(CPipeWriteEndPoint &write_pipe)
{
	try
	{
		...
		mrpt::utils::variant<mrpt::poses::CPose3D, mrpt::poses::CPose2D> var1(std::move(pose1));
		mrpt::utils::variant<mrpt::poses::CPose3D, mrpt::poses::CPose2D> var2(std::move(pose2));
