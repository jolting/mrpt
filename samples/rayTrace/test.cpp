 ...
 * color to it
 */
void configRandom(CRenderizable &obj)	{
	obj.setColor(MYRAND1(),MYRAND1(),MYRAND1(),MYRANDG(0.75,0.25));
	obj.setPose(randomPose());
}

void guideLines(const CPose3D &base,CSetOfLines &lines,float dist)	{
...
void generateObjects(CSetOfObjects &world)	{
	...
	CSphere sph(MYRANDG(5,1));
	configRandom(sph);
	world.insert(sph);

	CTexturedPlane pln(MYRANDG(10,-10),MYRANDG(10),MYRANDG(10,-10),MYRANDG(10));
	configRandom(pln);
	world.insert(pln);

	for (size_t i=0;i<5;i++)	{
		CPolyhedron poly=CPolyhedron::CreateRandomPolyhedron(MYRANDG(2,2));
		configRandom(poly);
		world.insert(poly);
	}

	CCylinder cil=CCylinder(MYRANDG(3.0,3.0),MYRANDG(3.0,1.0),MYRANDG(2.0f,3.0f),50,1);
	configRandom(cil);
	world.insert(cil);
...
void display()	{
	...
	COpenGLScene scene1;
	opengl::CGridPlaneXY plane1(-20,20,-20,20,0,1);
	...
	/*
	CSetOfLines untraced=CSetOfLines::Create();
	...
	*/
	...
	COpenGLScene &scene2=window2.get3DSceneAndLock();
	scene2.insert(aom);
	opengl::CGridPlaneXY plane2(-20,20,-20,20,0,1);
	plane2.setColor(GRID_R,GRID_G,GRID_B);
	scene2.insert(plane2);
	scene2.insert(CAxis(-5,-5,-5,5,5,5,2.5,3,true));
